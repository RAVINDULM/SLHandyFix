...
exports.getCustomerJobADByID = (req, res) => {
  ...
  CustomerJobADModel.getAllcustomerJobADByID(
    req.params.id,
    (err, customerJobAD) => {
...
exports.createcustomerJobAD = (req, res) => {
  ...
  if (req.body.constructor === Object && Object.keys(req.body).length === 0) {
    ...
    CustomerJobADModel.createcustomerJobAD(
...
exports.deletecustomerJobAD = (req, res) => {
  CustomerJobADModel.deletecustomerJobAD(
    req.params.id,
    (err, customerJobAD) => {
      if (err) res.send(err);
      res.json({
        success: true,
        message: "job advertisement deleted successully!",
      });
    }
  );
};
...
exports.RegisterCustomer = async (req, res) => {
  console.log("controller RegisterCustomer called: ", req.body);
  const customerReqData = req.body;
  console.log("RegisterCustomer", customerReqData);

  //   await CustomerJobADModel.RegisterCustomer(12, 2);
  //   res.send();

  if (req.body.constructor === Object && Object.keys(req.body).length === 0) {
    res.send(400).send({ success: false, message: "Please fill all fields" });
  } else {
    //   CustomerJobADModel.RegisterCustomer(12, 2);
    CustomerJobADModel.RegisterCustomer(
      customerReqData,
      (err, customerResData) => {
        if (err) res.send(err);
        res.json({
