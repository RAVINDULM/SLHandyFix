...
exports.registerManager = async (req, res) => {
    ...
    bcrypt.hash(passwordMang, 10).then(async (hash) => {
        const password = hash
        const result = await UserModel.createManageruser(req.body, contact_no
            , usertype, password)
        console.log(hash)
        if (result == false) {
            res.status(400).send()
            ...
            client.messages
                ...
                .then(message => console.log(message.sid));
            res.send({ message: "Manger Add Succesfully", userId: result })
        }
