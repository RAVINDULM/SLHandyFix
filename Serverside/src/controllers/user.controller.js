...
exports.register = (req, res) => {
  // const name =
  const { name, password, email, contact_no, usertype } = new UserModel(
    req.body
  );
...
exports.callOTP = (req, res) => {
  ...
  UserModel.getUserBycontact({ contact_no: contact }, (err, user) => {
    ...
    if (err) {
      ...
      otpGenerate = otpGenerator.generate(4, {
...
exports.ResetPassword = (req, res) => {
  ...
  bcrypt.hash(sendpass, 10).then((hash) => {
    UserModel.updatePasswordUserBycontact(
      { contact_no: sendcontact, password: hash },
      (err) => {
        if (err) {
          res.send(err);
        } else {
